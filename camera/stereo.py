...
def compute_left_disparity_map(img_left, img_right):
    ...
    disp_left = left_matcher_SGBM.compute(
        img_left, img_right).astype(np.float32) / 16
...
def locate_obstacle_in_image(image, obstacle_image):
    ...
    cross_corr_map = cv2.matchTemplate(
        image, obstacle_image, method=cv2.TM_CCOEFF)
...
def calculate_nearest_point(depth_map, obstacle_location, obstacle_img):
    ...
    obstacle_depth = depth_map_left[obstacle_min_x_pos:obstacle_max_x_pos,
